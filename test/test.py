import asyncio
import logging
from ..src.memory_tools_client.memory_tools_client import MemoryToolsClient, Query, OrderByClause, Aggregation
...
async def run_tests():
    ...
    try:
        ...
        print(f"✔ Success: Connected and authenticated as: {client_root.get_authenticated_username()}")
        ...
        print(f"✔ Success: Tried to create _system collection: {create_system_response}")
    except Exception as error:
    ...
    try:
        ...
        print(f"✔ Success: Connected and authenticated as: {client_admin.get_authenticated_username()}")
        ...
        if retrieved_main_item.found and retrieved_main_item.value == test_value:
            print(f"✔ Success: Key '{test_key}' retrieved from main store: {retrieved_main_item.value}")
        else:
            print(f"✖ Error: Failed to retrieve or verify key '{test_key}' from main store.")
        ...
        if updated_retrieved_main_item.found and updated_retrieved_main_item.value == updated_value:
            print(f"✔ Success: Key '{test_key}' retrieved after update: {updated_retrieved_main_item.value}")
        else:
            print(f"✖ Error: Failed to retrieve or verify updated key '{test_key}' from main store.")
        ...
        if not non_existent_item.found:
            print("✔ Success: Non-existent key 'non_existent_key' not found as expected.")
        else:
            print("✖ Error: Non-existent key 'non_existent_key' found unexpectedly.")

        # 5. Delete the key-value pair
        start_time = asyncio.get_event_loop().time()
        await client_admin.set(test_key, None, 0) # Setting to None with 0 TTL effectively deletes it
        end_time = asyncio.get_event_loop().time()
        ...
        if test_collection_name in collections_after_create.names:
            print(f"✔ Success: Collection '{test_collection_name}' found in list after creation.")
        else:
            print(f"✖ Error: Collection '{test_collection_name}' not found in list after creation.")
        ...
        print(f"✔ Success: Item '{item_key2}' set in '{test_collection_name}' with TTL.")
        ...
        start_time = asyncio.get_event_loop().time()
        all_items_after_set_many = await client_admin.collection_item_list(test_collection_name)
        end_time = asyncio.get_event_loop().time()
        print(f"CollectionItem_List_AfterSetMany: {end_time - start_time:.3f}s")
        found_keys = list(all_items_after_set_many.items.keys())
        expected_keys = [item_key1, item_key2, "many_item_a", "many_item_b", "many_item_c"]
        if len(found_keys) == len(expected_keys) and sorted(found_keys) == sorted(expected_keys):
        ...
        start_time = asyncio.get_event_loop().time()
        await client_admin.collection_item_delete_many(test_collection_name, keys_to_delete)
        end_time = asyncio.get_event_loop().time()
        ...
        start_time = asyncio.get_event_loop().time()
        items_after_delete_many = await client_admin.collection_item_list(test_collection_name)
        end_time = asyncio.get_event_loop().time()
        ...
        if (
            len(keys_after_delete_many) == 3 and
            sorted(keys_after_delete_many) == sorted(expected_keys_after_delete_many)
        ):
            print("✔ Success: Items were correctly deleted. Remaining keys:", keys_after_delete_many)
        else:
            print("✖ Error: Deletion of multiple items failed. Remaining keys:", keys_after_delete_many)
        # --- END NEW TEST ---
        ...
        try:
            ...
            start_time = asyncio.get_event_loop().time()
            query_result = await client_admin.collection_query(query_collection_name, Query(
            ...
            expected_names1 = ["Adonay", "David Blanco", "Juan Pérez", "María García", "Sofía Ruiz"]
            if (
                len(query_result) == len(expected_names1) and
                [u["name"] for u in query_result] == expected_names1
            ):
            ...
            expected_names2 = sorted(["Adonay", "Pedro López", "Sofía Ruiz", "María García", "David Blanco"])
            ...
            start_time = asyncio.get_event_loop().time()
            count_query_result = await client_admin.collection_query(query_collection_name, Query(
                count=True,
            ...
            if count_query_result.get("count") == 9:
                print(f"✔ Success: Query 3 count matches expected count ({count_query_result.get('count')}).")
            else:
                print(f"✖ Error: Query 3 count mismatch. Expected 9, got {count_query_result.get('count')}.")
            ...
            expected_names4 = ["Adonay", "Carlos Sanz", "Elena Castro", "David Blanco", "Sofía Ruiz"]
            if (
                len(query_result) == len(expected_names4) and
                [u["name"] for u in query_result] == expected_names4
            ):
            ...
            if (
                isinstance(query_result, list) and
                all(isinstance(item.get("city"), str) and isinstance(item.get("average_age"), (int, float)) for item in query_result)
            ):
            ...
            expected_names7 = sorted(["Adonay", "Juan Pérez", "María García", "Ana Díaz", "Sofía Ruiz", "Elena Castro"])
            ...
            expected_cities = sorted(["Madrid", "Barcelona", "Valencia", "Sevilla", "Bilbao"])
            ...
            print(f"✖ Error: Failed to perform collectionQuery on '{query_collection_name}': {query_error}")
        finally:
        ...
        if test_collection_name not in collections_after_delete.names:
            print(f"✔ Success: Collection '{test_collection_name}' not found in list after deletion.")
        else:
            print(f"✖ Error: Collection '{test_collection_name}' found in list after deletion unexpectedly.")

    except Exception as error:
        print("\n--- UNEXPECTED ERROR DURING TESTS ---")
        print(f"Error: {error}")
        if "Authentication failed" in str(error):
            print("Hint: This error indicates the server did not recognize the authentication command or rejected it. Check the CMD_AUTHENTICATE value on both sides (Go and Python) and the UserInfo struct in Go for invisible characters.")
            print("Also ensure your 'admin' user in the Go DB doesn't have 'IsRoot: true' by mistake.")
        elif "Connection refused" in str(error):
            print("Hint: Is the MemoryTools Go server running and listening on the specified host and port?")
    finally:
        print("\nClosing connection...")
        if client_admin:
            client_admin.close()
